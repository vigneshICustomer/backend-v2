  ...
  res.status(200).json({
    ...
    endpoints: {
      ...
      audience_segment_management: {
        'POST /audience/fetch-all-segments': 'Get all segments for organization',
        ...
        'POST /segment/segment-filter-criteria': 'Preview segment based on conditions',
        'POST /audience/segment-filter-criteria-count': 'Get segment filter count',
        ...
        'POST /audience/fetch-segments-modelId': 'Get all segments for specific model',
      },
      insights_analytics: {
      ...
      data_enrichment: {
        'POST /insightsRoutes/enrichColumns': 'Enrich columns with additional data',
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      ...
      if (environment.NODE_ENV === 'development') {
        console.log(`🌐 CORS origins: ${environment.ALLOWED_ORIGINS.join(', ')}`);
      }
    });
  } catch (error) {
