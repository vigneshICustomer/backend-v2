...
const schemaName =
  process.env.NODE_ENV === "production" ? "audience_hub" : "audience_hub";
...
export const relationships = schema.table("relationships", {
  id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
  fromObjectId: integer("from_object_id")
    .notNull()
    .references(() => objects.id),
  toObjectId: integer("to_object_id")
    .notNull()
    .references(() => objects.id),
...
export const cohorts = schema.table("cohorts", {
  ...
  filters: jsonb("filters")
    .notNull()
    .default('{"companyFilters": [], "contactFilters": []}'),
